	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if (opr.equals("addComment")) {// 添加评论
				...
				pageObj.setPageSize(30); // 设置每页显示条数
				if (tid == null || (tid = tid.trim()).length() == 0) {
					list4 = newsService.findAllNewsByKey(keyname);
				...
				try {
					...
					if (result == 0) {
						...
						if (role.equals("管理员")) {
							...
							out.print("location.href=\"" + contextPath + "/util/news?opr=editorlist\";");
				...
				String catid[] = { "news_society", "news_world", "news_military", "news_sports", "news_entertainment",
						"news_fashion", "news_finance", "news_tech", "news_health", "news_car", "news_edu",
						"news_house", "news_home", "news_travel", "news_culture", "funny", "news_history", "news_food",
						"movie", "news_game", "news_comic", "emotion" };
				...
				request.setAttribute("topics", topicService.findAllTopics());
				request.getRequestDispatcher("/newspages/autoGet.jsp").forward(request, response);
			}
								...
								try {
									...
									if (result == 0) {
										...
										if (role.equals("管理员")) {
											...
											out.print(
													"location.href=\"" + contextPath + "/util/news?opr=editorlist\";");
